...
do
    ...
    library = utility.table {
        flags = {},
        toggled = true,
        accent = Color3.fromRGB(184, 0, 3),
        outline = { Color3.fromRGB(184, 0, 3), Color3.fromRGB(50, 0, 0) },
    ...
    function library:Watermark(options)
        ...
        local watermark = utility.create("TextLabel", {
            ...
            BorderColor3 = Color3.fromRGB(255, 43, 43),
    ...
    function library:New(options)
        ...
        local sizeY = options.sizeY or 600
        ...
        function windowtypes:Page(options)
            ...
            function pagetypes:Section(options)
                ...
                local glow = utility.create("Frame", {
                    ZIndex = 8,
                    Size = UDim2.new(1, 0, 0, 1),
                    BorderColor3 = Color3.fromRGB(255, 41, 41),
                    BorderSizePixel = 0,
                    BackgroundColor3 = library.accent,
                    Parent = section
                })
                ...
                function sectiontypes:ColorPicker(options)
                    ...
                    local function toggle()
                        if not switchingtabs then
                            ...
                            local textcolor = toggled and library.accent or Color3.fromRGB(255, 41, 41)
                            utility.tween(rainbowtxt, { 0.2 }, { TextColor3 = textcolor })
...
local ScriptProperties = {
    ScriptName = "DeepHub.exe",
    ScriptSizeOne = 800,
    ScriptSizeTwo = 900,
    ScriptAccent = Color3.fromRGB(255, 41, 41),


    Perms = { 246220626, 2415886442, 2284385613, 2415886442 },
    Owners = { "mashinaj010", "Fermeris_14" },
    Admins = {},
...
Library:Initialize()
local Window = Library:New({ Name = ScriptProperties.ScriptName, Accent = Color3.fromRGB(255, 41, 41) })

local Pages = {
    Farming = Window:Page({ Name = "Farming" }),
    ZoneCast = Window:Page({ Name = "ZoneCast" }),
    Buy = Window:Page({ Name = "Buy" }),
    Visual = Window:Page({ Name = "Visual" }),
    Teleport = Window:Page({ Name = "Teleport" }),
    Misc = Window:Page({ Name = "Misc" }),
    ConfigSec = Window:Page({ Name = "Config" })
}

local Sections = {
    Fishing = Pages.Farming:Section({ Name = "Fishing", Side = "Left", Max = 5 })
}

local CatchType = "unknown"
local AutoCatch = false
local AutoShake = false
local AutoShakeSpeed = 0.5
local AutoCast = false
local CastPower = 100

Autocast = Sections.Fishing:Toggle(
        {
        Name = "Auto Cast",
        Default = false,
        Pointer = "14",
...
castpower = Sections.Fishing:Slider(
        {
        Name = "Cast Power",
        Minimum = 1,
        Maximum = 100,
...
Autoshake = Sections.Fishing:Toggle(
        {
        Name = "Auto Shake",
        Default = false,
        Pointer = "12",
...
autoShakespeed = Sections.Fishing:Slider(
        {
        Name = "Auto Shake Speed",
        Minimum = 0,
        Maximum = 1,
        Default = 0.5,
        Decimals = 0.1,
        Pointer = "11",
        callback = function(state)
            AutoShakeSpeed = state
        end
    }
)

RunService.Heartbeat:Connect((function(delta1)
	if AutoCast == true then
		if Rod then
			if progress == false then
				Rod.events.cast:FireServer(CastPower)
				task.wait(0.1)
			end
		end
	end
...
RunService.Heartbeat:Connect((function(delta1)
	if Rod then
        if Rod.values.casted == true then
            progress = true
        elseif Rod.values.casted == false then
            progress = false
        end
    end
end))

local NewRod = game.Players.LocalPlayer.Character:FindFirstChildWhichIsA('Tool')
